      ...
      <Box component='footer' sx={{ display: 'flex', flexFlow: 'row nowrap', justifyContent: 'space-around', marginTop: '24px',p:'16px' }}>
        ...
        sx={{ mr:'16px',width:'15%', marging: '0', '&>.MuiInputBase-root': { marginTop: 0 } }}>
          ...
          <Select
            ...
            sx={{textTransform:'uppercase',fontSize:'1em',mr:'1em'}}
          >
            {selectCountry.map((country) => {
              return <MenuItem key={country} value={country} sx={{ textTransform: 'uppercase',fontSize:'2em' }}>{country}</MenuItem>
            })}
          </Select>
        </FormControl>
            ...
            <StyledList key={option} sx={{width:'18%'}}>
              {option.map((value, index) => {
                ...
                return <StyledListItem key={index} sx={{  '&:hover': {
                  ...
                  <ListItemText primary={value} sx={{fontSize:'1em'}} />
                </StyledListItem>
        ...
        <Box sx={{ mt: '24px', pr: '16px', display: 'flex', flexDirection: 'column', alignItems: 'flex-start',width:'18%' }}>
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            <CopyrightIcon sx={{ fontSize:'1em'}} />
            <Typography sx={{ fontSize:'1em' , ml: '2px' }}>{trademark}</Typography>
          </Box>

          <Typography sx={{ fontSize: '1em' }}>{tmDescription}</Typography>
        </Box>
      </Box>
    </Box>
  )
